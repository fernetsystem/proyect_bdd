...
public class Pedidos2 extends Connect implements ActionListener{
	public JFrame f;
	JLabel lb1,lb2,bak;
	...
	public void llena(){
	try{
		String query="select p.idfactura,fecha,idpedido,codigo,unidades,estado from pedidos p inner join facturas on p.idfactura=facturas.idfactura where idsucursal=2";
	...
	try{
		String query="select p.idfactura,fecha,idpedido,codigo,unidades,estado from pedidos p inner join facturas on p.idfactura=facturas.idfactura";
  		query=query+" where idsucursal=2 and p.idfactura like'%"+palabra+"%' or fecha like'%"+palabra+"%' or idpedido like'%"+palabra+"%' or codigo like'%"+palabra+"%' or unidades like'%"+palabra+"%' or estado like'%"+palabra+"%'";
  		stmt =conexion.prepareStatement(query);
  		tabla=stmt.executeQuery();
  		table.setModel(DbUtils.resultSetToTableModel(tabla));	
		}catch(SQLException e){JOptionPane.showMessageDialog(null,e);	}
	...
	public void obtenerExistenciaDeS2(){
		try{
		String query="select * from prendas_sucursal2 where codigo="+codigo;
  		stmt =conexion.prepareStatement(query);
  		tabla=stmt.executeQuery();
  		while(tabla.next()){
	...
	if(existenciaS1==null){
		...
		try{
			String query="INSERT INTO prendas_sucursal2 VALUES("+codigo+",'"+nombre+"','"+desc+"','"+color+"','"+depto+"',"+unidades+","+nuevoPrecio+")";
  			stmt =conexion.prepareStatement(query);
  			int retorno = stmt.executeUpdate();
    		if(retorno == 1){JOptionPane.showMessageDialog(null,"SE INSERTO UN NUEVO PRODUCTO A LA S2");}
    		if(retorno == 0){JOptionPane.showMessageDialog(null,"OCURRIO UN ERRO AL INSERTAR A LA S2");}
		}catch(SQLException e){JOptionPane.showMessageDialog(null,e);}
	}else{
		...
		try{
			String query="UPDATE prendas_sucursal2 SET existencia="+nuevaExistencia+" where codigo="+codigo;
  			stmt =conexion.prepareStatement(query);
  			int retorno = stmt.executeUpdate();
    		if(retorno == 1){JOptionPane.showMessageDialog(null,"SE ACTUALIZO LA EXISTENCIA EN S2");}
